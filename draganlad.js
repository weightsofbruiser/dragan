/* ============================
   dragan.js (Persona-ish pass)
   COPY-PASTE REPLACEMENT
   Changes: calmer idle FX, faster decay, less constant lock-on.
   ============================ */
...
function microFxKick(str = 1) {
  // Persona-ish: calm idle, strong spikes during events
  fxPulse = Math.max(fxPulse, 0.10 * str + Math.random() * 0.10);
}
...
setInterval(() => {
  ...
  // Less constant lock-on = cleaner Persona idle
  if (Math.random() < 0.40) pingLock();
...
function breathe() {
  ...
  // Faster FX decay = cleaner idle (Persona-ish)
  fxPulse = Math.max(0, fxPulse - 0.040);
